    ...
    <div class="blank">
		...
		<%
        	...
        	try {
    			...
    			String sql = "SELECT appointment.ID, time, doctor.name, doctor.address FROM patient INNER JOIN appointment ON patient.ID = "
    		+ "appointment.patientID INNER JOIN doctor ON appointment.doctorID = doctor.ID WHERE patient.username = '"+ un +"' AND status = 'Confirmed'";
    			ResultSet rs = stmt.executeQuery(sql);
    			while(rs.next()){
    	%>
    	
			<form method="post" action="ViewPrescription">
                <div>
                ...
                <div class="infor">
                    <div class="infoleft">
                        ...
                        <ul class="name">
                            Doctor Name: <%= rs.getString(3) %>
                        </ul>
                        <ul class="address">
                            Doctor Address: <%= rs.getString(4) %>
                        </ul>
                    </div>

                    <div class="inforight">
                        <ul class="viewmore">         
                        	<input type="hidden" name="appointment" value="<%= rs.getInt(1) %>">                   
