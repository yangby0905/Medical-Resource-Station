	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(flag) {
			try {
				...
				if(rn != "") {
					sql = "UPDATE patient SET name = '"+ rn +"'";
					stmt.execute(sql);
				}
				
				if(age != "") {
					int a = Integer.parseInt(age);
					sql = "UPDATE patient SET age = '"+ a +"'";
					stmt.execute(sql);
				}
				...
				if(address != "") {
					sql = "UPDATE patient SET address = '"+ address +"'";
					stmt.execute(sql);
				}
				
				String phone = request.getParameter("phone");
				if(phone != "") {
					sql = "UPDATE patient SET phone = '"+ phone +"'";
					stmt.execute(sql);
				}
				
				String email = request.getParameter("email");
				if(email != "") {
					sql = "UPDATE patient SET email = '"+ email +"'";
					stmt.execute(sql);
				}
